...
% TODO: 全ての引数型に対してspecializeするわけではないことを説明
% 過剰なcode generationを避けるために，ヒューリスティックが用いられる
...
\footnotetext{
  Juliaは現在のところ逆方向の型推論（"backwards type flow"）を行うことができない\cite{julia-2012}．
  コンテナ型の変数のパラメータ型を変数宣言以降のコードの操作から
  推論することはできず，この場合\texttt{ints}には
  \texttt{[]}の型\texttt{Array\{Any,1\}}がそのまま与えられる．
}
