...
\footnotetext{
  ...
  過剰なcode generation（\ref{subsubsection:eval-process}）が行われてしまうことを避けている．
...
generic functionが呼ばれた時，そのメソッドの中から全ての引数の実行時の型\footnote{
...
\footnotetext{
  Juliaは現在のところ逆方向の型推論（"backwards type flow"）を行うことができない\cite{julia-2012}\ref{subsubsection:inference-algorithm}．
